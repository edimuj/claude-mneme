 ...
 * Can be called directly or spawned in background by maybeSummarize()
 *
 * Usage: node summarize.mjs <project-dir>
 *
 * Checks if log has exceeded threshold and runs AI summarization if needed.
 * Uses Claude Agent SDK with Haiku for compression.
 */
...
if (existsSync(lockFile)) {
  ...
  if (Date.now() - lockTime < 5 * 60 * 1000) {
    // Lock is fresh, another summarization is running
    process.exit(0);
  }
}
